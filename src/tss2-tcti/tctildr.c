...
TSS2_RC
tcti_from_init(TSS2_TCTI_INIT_FUNC init, const char *conf, TSS2_TCTI_CONTEXT **tcti) {
    TSS2_RC r;
    size_t  size;
...
TSS2_RC
tcti_from_info(TSS2_TCTI_INFO_FUNC infof, const char *conf, TSS2_TCTI_CONTEXT **tcti) {
    TSS2_RC r;
 ...
 */
TSS2_RC
tctildr_conf_parse_alloc(const char *name_conf, char **name, char **conf) {
    size_t  combined_length;
    TSS2_RC rc;
...
TSS2_RC
tctildr_transmit(TSS2_TCTI_CONTEXT *tctiContext,
                 size_t             command_size,
...
TSS2_RC
tctildr_receive(TSS2_TCTI_CONTEXT *tctiContext,
    ...
    return Tss2_Tcti_Receive(ldr_ctx->tcti, response_size, response_buffer, timeout);
}
TSS2_RC
tctildr_cancel(TSS2_TCTI_CONTEXT *tctiContext) {
    TSS2_TCTILDR_CONTEXT *ldr_ctx = tctildr_context_cast(tctiContext);
    if (ldr_ctx == NULL) {
        return TSS2_TCTI_RC_BAD_REFERENCE;
    }
...
TSS2_RC
tctildr_get_poll_handles(TSS2_TCTI_CONTEXT     *tctiContext,
...
TSS2_RC
tctildr_set_locality(TSS2_TCTI_CONTEXT *tctiContext, uint8_t locality) {
    TSS2_TCTILDR_CONTEXT *ldr_ctx = tctildr_context_cast(tctiContext);
    if (ldr_ctx == NULL) {
        return TSS2_TCTI_RC_BAD_REFERENCE;
    }
...
TSS2_RC
tctildr_make_sticky(TSS2_TCTI_CONTEXT *tctiContext, TPM2_HANDLE *handle, uint8_t sticky) {
    TSS2_TCTILDR_CONTEXT *ldr_ctx = tctildr_context_cast(tctiContext);
    if (ldr_ctx == NULL) {
        return TSS2_TCTI_RC_BAD_REFERENCE;
    }
...
void
tctildr_finalize(TSS2_TCTI_CONTEXT *tctiContext) {
    TSS2_TCTILDR_CONTEXT *ldr_ctx = tctildr_context_cast(tctiContext);
    if (ldr_ctx == NULL) {
        return;
    }
...
TSS2_RC
copy_info(const TSS2_TCTI_INFO *info_src, TSS2_TCTI_INFO *info_dst) {
...
TSS2_RC
tctildr_init_context_data(TSS2_TCTI_CONTEXT *tctiContext, const char *name, const char *conf) {
...
TSS2_RC
Tss2_TctiLdr_Initialize_Ex(const char *name, const char *conf, TSS2_TCTI_CONTEXT **tctiContext) {
    TSS2_TCTILDR_CONTEXT *ldr_ctx = NULL;
    TSS2_RC               rc;
    void                 *dl_handle = NULL;
...
TSS2_RC
Tss2_TctiLdr_Initialize(const char *nameConf, TSS2_TCTI_CONTEXT **tctiContext) {
    char   *name = NULL;
    char   *conf = NULL;
    TSS2_RC rc;
...
__attribute__((weak)) const TSS2_TCTI_INFO tss2_tctildr_info = {
    ...
    .config_help = "TCTI to load and its configuration. Either"
                   ...
                   "Where child_name: if not empty, tctildr will try to dynamically load the child "
                   "tcti library in the following order:\n"
                   "   * <child_name>\n"
...
__attribute__((weak)) const TSS2_TCTI_INFO *
Tss2_Tcti_Info(void) {
